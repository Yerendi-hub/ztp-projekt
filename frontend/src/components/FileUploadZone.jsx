...
export default function FileUploadZone({ files, setFiles }) {
  ...
  const handleFileChange = (e) => {
    ...
    setFiles(prevFiles => {
      ...
      return [
        ...prevFiles,
        ...chosenFiles.filter(file => !existing.has(`${file.name}:${file.size}:${file.lastModified}`))
      ];
    });
    e.target.value = '';
  };
      ...
      {files.length > 0 && (
        <div className="border-2 border-[#2d4a43] p-3 bg-[#e2decb]/50 text-[13px] shrink-0 mt-2">
          ...
          <div className="max-h-36 overflow-y-auto pr-1">
            {files.map(file => (
              <div key={`${file.name}:${file.size}:${file.lastModified}`} className="flex justify-between items-center bg-white/90 p-2 mb-1.5 border border-[#2d4a43]/30 shadow-[1px_1px_0px_0px_rgba(0,0,0,0.05)]">
                <span className="truncate pr-2 flex items-center gap-2 font-semibold text-[#2d4a43]">
                  <FileText className="w-4 h-4 text-[#5b8276] shrink-0" /> {file.name}
                </span>
                <button
                  type="button"
                  onClick={(e) => {
